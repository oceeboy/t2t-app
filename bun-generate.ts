...
const createModule = (moduleName: string) => {
  ...
  if (existsSync(modulePath)) {
    ...
    subfolders.forEach((folder) => {
      ...
      if (!existsSync(folderPath)) {
        ...
        console.log(
          `✅ Created missing folder: ${moduleName}/${folder}`,
        );
        allPresent = false;
      }
    });
  ...
  subfolders.forEach((folder) => {
    ...
    console.log(
      `📂 Created folder: ${moduleName}/${folder}`,
    );
  });
...
const updateModuleIndex = (
  ...
  const exportStatements = subfolders
    .filter((folder) =>
      existsSync(join(modulePath, folder)),
...
const updateModulesIndex = () => {
  ...
  const existingModules = readdirSync(
    BASE_MODULES_PATH,
  )
...
// CLI Handling commands
const args = process.argv.slice(2);
if (args.length !== 2 || args[0] !== 'modules') {
  console.log(
    '❌ Usage: bun bun-generate.ts modules <module-name>',
  );
  process.exit(1);
}
